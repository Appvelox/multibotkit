...
class FileBasedObject(BaseModel):
    file_id: str = Field(
        ...,
        title="Identifier for this file, which can be used to download or reuse the file",
    )
    file_unique_id: str = Field(
        ...,
        title="Unique identifier for this file, which is supposed to be the same over time and for different bots",
    )
...
class Location(BaseModel):
    ...
    horizontal_accuracy: Optional[float] = Field(
        None,
        title="The radius of uncertainty for the location, measured in meters; 0-1500",
    )
    live_period: Optional[int] = Field(
        None,
        title="Time relative to the message sending date, during which the location can be updated, in seconds",
    )
    ...
    proximity_alert_radius: Optional[int] = Field(
        None,
        title="Maximum distance for proximity alerts about approaching another chat member, in meters",
    )
...
class MaskPosition(BaseModel):
    point: MaskPositionPoint = Field(
        ...,
        title="The part of the face relative to which the mask should be placed. One of “forehead”, “eyes”, “mouth”, "
        "or “chin”",
    )
    x_shift: float = Field(
        ...,
        title="Shift by X-axis measured in widths of the mask scaled to the face size, from left to right",
    )
    y_shift: float = Field(
        ...,
        title="Shift by Y-axis measured in heights of the mask scaled to the face size, from top to bottom",
    )
...
class Sticker(FileBasedObject):
    ...
    mask_position: Optional[MaskPosition] = Field(
        None, title="For mask stickers, the position where the mask should be placed"
    )
...
class VideoNote(FileBasedObject):
    length: int = Field(
        ...,
        title="Video width and height (diameter of the video message) as defined by sender",
    )
    duration: int = Field(
...
class Audio(FileBasedObject):
    ...
    performer: Optional[str] = Field(
        None, title="Performer of the audio as defined by sender or by audio tags"
    )
    ...
    thumb: Optional[Photo] = Field(
        None, title="Thumbnail of the album cover to which the music file belongs"
    )
...
class Chat(BaseModel):
    ...
    type: ChatType = Field(
        ...,
        title="Type of chat, can be either “private”, “group”, “supergroup” or “channel”",
    )
    title: Optional[str] = Field(
    ...
    username: Optional[str] = Field(
        None, title="Username, for private chats, supergroups and channels if available"
    )
...
class Message(BaseModel):
    ...
    caption: Optional[str] = Field(
        None, title="Caption for the animation, audio, document, photo, video or voice"
    )
    ...
    video_note: Optional[VideoNote] = Field(
        None, title="Message is a video note, information about the video message"
    )
    ...
    location: Optional[Location] = Field(
        None, title="Message is a shared location, information about the location"
    )
...
class CallbackQuery(BaseModel):
    ...
    inline_message_id: Optional[int] = Field(
        None,
        title="Identifier of the message sent via the bot in inline mode, that originated the query",
    )
    chat_instance: Optional[str] = Field(
        None,
        title="Global identifier, uniquely corresponding to the chat to which the message with the callback button "
        "was sent",
    )
    data: Optional[str] = Field(None, title="Data associated with the callback button")
    game_short_name: Optional[str] = Field(
        None,
        title="Short name of a Game to be returned, serves as the unique identifier for the game",
    )

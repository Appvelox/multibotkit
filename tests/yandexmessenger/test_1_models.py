...
def test_incoming_models_sender():
    ...
    parsed_sender = Sender.parse_obj(sender_dict)
...
def test_incoming_models_chat():
    ...
    private_json = private_chat.json()
...
def test_incoming_models_image():
    ...
    parsed_image = Image.parse_obj(image_dict)
...
def test_incoming_models_file():
    ...
    parsed_file = File.parse_obj(file_dict)
...
def test_incoming_models_update():
    ...
    update_json = update.json(by_alias=True)
...
def test_outgoing_models_inline_keyboard():
    ...
    parsed_keyboard = InlineKeyboard.parse_obj(keyboard_dict)
    assert len(parsed_keyboard.buttons) == 1
    assert len(parsed_keyboard.buttons[0]) == 2
    assert parsed_keyboard.buttons[0][0].text == "Button 1"
...
def test_outgoing_models_send_text():
    ...
    params = SendTextParams(
        text="Test message", login="test_user", disable_notification=True, important=False
    )
    ...
    group_dict = group_params.dict(exclude_none=True)
...
def test_outgoing_models_responses():
    ...
    updates_response = UpdatesResponse(ok=True, updates=[Update.parse_obj(update_dict)])
